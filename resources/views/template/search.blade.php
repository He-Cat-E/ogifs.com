        ...
        <div class="">
            ...
            <div class="the-filter-gallery mt-3">
                <div class="mosaicflow">
                    ...
                    @if($postdetail->posts != NULL)
                    <a href="{{Route('detail.page', ['id' => $postdetail->posts->id])}}" class="mosaicflow__item" onclick="countView({{ $postdetail->posts->id }})">
                        <div class="position-relative">
                            @if (in_array(pathinfo($postdetail->posts->gif, PATHINFO_EXTENSION), ['mp4', 'avi', 'mov', 'mkv', 'm4v', 'webm', 'wmv', 'flv', 'ogg', 'ogv', '3gp']))
                            {{-- Video --}}
                            <video width="100%" height="100%" loop muted autoplay playsinline>
                                <source src="{{ asset('/Gifs/'.$postdetail->posts->gif) }}" type="video/mp4">
                                Your browser does not support the video tag.
                            </video>
                            @else
                            {{-- Image --}}
                            <img src="{{ asset('/Gifs/'.$postdetail->posts->gif) }}">
                            @endif

                            <div class="position-absolute text-purple likes-adj">
                                <i class="fa-solid fa-heart"></i> <span>{{ count($postdetail->posts->likes) }}</span>
                            </div>
                            <div class="position-absolute text-purple views-adj">
                                <i class="fa-solid fa-eye"></i> <span>{{ count($postdetail->posts->views) }}</span>
                            </div>
                        </div>
